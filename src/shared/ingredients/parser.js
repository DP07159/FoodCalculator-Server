...
function cleanIngredientName(value) {
    const unitPattern =
        "kg|g|gr|gramm|ml|l|liter|stk\\.?|stück|stueck|dose|dosen|glas|gläser|glaeser|packung|packungen|pkg|el|esslöffel|essloeffel|tl|teelöffel|teeloeffel|prise|prisen";
    ...
    const cleaned = String(value || "")
        ...
        .replace(
            new RegExp(
                `\\b(?:a|à)\\s*${amountPattern}\\s*(${unitPattern})\\b`,
                "gi"
            ),
            " "
        )
        .replace(
            new RegExp(
                `(^|[\\s,(])${amountPattern}\\s*(${unitPattern})\\b`,
                "gi"
            ),
            " "
        )
        .replace(
            new RegExp(
                `(^|[\\s,(])(${unitPattern})\\s*${amountPattern}\\b`,
                "gi"
            ),
            " "
        )
...
function getContainerMultiplier(rawText, physicalMatch) {
    const textBeforePhysical = String(rawText || "").slice(
        0,
        physicalMatch?.start ?? 0
    );

    const containerUnitPattern =
        "stk\\.?|stück|stueck|dose|dosen|glas|gläser|glaeser|packung|packungen|pkg";

    const containerMatches = findAmountUnitMatches(
        textBeforePhysical,
        containerUnitPattern
    );
    ...
    const lastContainer =
        containerMatches[containerMatches.length - 1];

    const between = String(rawText || "")
        .slice(lastContainer.end, physicalMatch.start)
        .toLowerCase();

    const hasPerUnitHint =
        /(?:\b(?:a|à|je|pro)\b|\/|per)/i.test(between);

    const isCloseEnough = between.length <= 50;

    if (!hasPerUnitHint && !isCloseEnough) {
        return 1;
    }
...
function findAmountUnitInIngredient(rawText) {
    ...
    const physicalUnitPattern =
        "kg|g|gr|gramm|ml|l|liter";

    const containerUnitPattern =
        "stk\\.?|stück|stueck|dose|dosen|glas|gläser|glaeser|packung|packungen|pkg|el|esslöffel|essloeffel|tl|teelöffel|teeloeffel|prise|prisen";

    const physicalMatches = findAmountUnitMatches(
        rawText,
        physicalUnitPattern
    );
    ...
    if (physicalMatches.length) {
        ...
        const selected =
            /abtropf|abgetropft|netto|einwaage/.test(text)
                ? physicalMatches[physicalMatches.length - 1]
                : physicalMatches[0];

        return {
            ...selected,
            multiplier: getContainerMultiplier(rawText, selected)
        };
    }
    ...
    const amountOnlyRegex = new RegExp(
        `(^|[\\s,(])(${amountPattern})(?=\\s|$)`,
        "i"
    );
...
function parseIngredientLine(line) {
    ...
    if (amountUnit) {
        ...
        foodName =
            `${rawText.slice(0, amountUnit.start)} ` +
            `${rawText.slice(amountUnit.end)}`;
    }
    ...
    const normalizedAmount = convertIngredientAmount(
        amount,
        unit
    );
