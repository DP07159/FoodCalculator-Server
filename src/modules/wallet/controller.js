...
async function getFoodMomentLinks(req,res,next){try{const result=await service.getFoodMomentLinkOptions({workspaceId:req.workspaceId,userId:req.auth.user.id,publicId:req.params.publicId});if(result.notFound)return res.status(404).json({error:'Wallet-Eintrag nicht gefunden.'});res.json(result.value);}catch(error){next(error);}}
async function updateFoodMomentLinks(req,res,next){try{const result=await service.setFoodMomentLinks({workspaceId:req.workspaceId,userId:req.auth.user.id,publicId:req.params.publicId,foodMomentPublicIds:req.body.food_moment_public_ids});if(result.notFound)return res.status(404).json({error:'Wallet-Eintrag nicht gefunden.'});if(result.error)return res.status(400).json({error:result.error});res.json(result.value);}catch(error){next(error);}}

module.exports = { list, preview, create, update, remove, getWorkspaceAssignments, updateWorkspaceAssignments, getRecipeLinks, updateRecipeLinks, addRecipeLink, getItemsForRecipe, getFoodMomentLinks, updateFoodMomentLinks };
