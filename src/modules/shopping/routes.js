...
router.get("/shopping-list", async (req,res,next)=>{ try { await service.reconcileIncomingCollaborativeList(req.workspaceId,req.auth.user.id); res.json(await service.getList(req.workspaceId)); } catch(e){ next(e); } });
...
router.patch("/shopping-list/group", async (req,res,next)=>{ try { const r=await service.setGroupCompleted(req.body||{},req.workspaceId,req.auth.user.id); if(r.error)return res.status(400).json({error:r.error}); res.json(r.value); } catch(e){ next(e); } });
