...
router.post("/shopping-list/import/week", async (req,res,next)=>{ try { const r=await service.importWeek(req.body?.start_date,req.workspaceId,req.auth.user.id); if(r.error)return res.status(400).json({error:r.error}); res.json(r.value); } catch(e){ next(e); } });
