...
router.delete("/shopping-list/completed", async (req,res,next)=>{ try { const result=await service.clearCompleted(req.workspaceId); await service.syncAllShoppingShares(req.workspaceId,req.auth.user.id); res.json(result); } catch(e){ next(e); } });
