...
router.put("/recipe/:recipeId/links", async (req, res, next) => {
    try {
        const recipeId=intOrNull(req.params.recipeId); if(!recipeId)return res.status(400).json({error:"Ungültige Rezept-ID."});
        const recipe=await get(`SELECT r.id FROM recipes r WHERE r.id=? AND (r.workspace_id=? OR EXISTS(SELECT 1 FROM recipe_workspace_assignments a WHERE a.recipe_id=r.id AND a.workspace_id=?)) LIMIT 1`,[recipeId,req.workspaceId,req.workspaceId]);
        if(!recipe)return res.status(404).json({error:"Rezept nicht gefunden."});
        ...
        const targetIds=requested.map(id=>Number(byPublic.get(id).id));
        ...
        try {
            ...
            for(const targetId of targetIds) await run(`INSERT OR IGNORE INTO food_moment_recipe_links(food_moment_id,recipe_id) VALUES(?,?)`,[targetId,recipeId]);
            const verifyRows=visibleIds.length?await all(`SELECT food_moment_id FROM food_moment_recipe_links WHERE recipe_id=? AND food_moment_id IN (${visibleIds.map(()=>'?').join(',')}) ORDER BY food_moment_id`,[recipeId,...visibleIds]):[];
            const actual=verifyRows.map(r=>Number(r.food_moment_id)).sort((a,b)=>a-b); const expected=[...targetIds].sort((a,b)=>a-b);
            if(actual.length!==expected.length||actual.some((id,i)=>id!==expected[i])) throw new Error('Persistenzprüfung der Food-Moment-Verknüpfungen fehlgeschlagen.');
            await run('COMMIT');
        } catch(error){await run('ROLLBACK').catch(()=>{});throw error;}
        const persistedRows=requested.map(id=>byPublic.get(id)).filter(Boolean);
        res.json({food_moments:await Promise.all(persistedRows.map(r=>hydrate(r,req.workspaceId))),available_food_moments:await Promise.all(availableRows.map(r=>hydrate(r,req.workspaceId))),verified:true});
    } catch(e){next(e);}
});
