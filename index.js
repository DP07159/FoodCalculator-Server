...
function normalizeWalletRow(item) {
    return {
        ...
        source_platform: item.source_platform || detectSourcePlatform(item.source_url),
        note: item.note || "",
...
app.post("/wallet", async (req, res) => {
    try {
        ...
        const title = titleInput || sourceUrl || "Neue Inspiration";
        const sourcePlatform = typeof req.body.source_platform === "string" && req.body.source_platform.trim()
            ? req.body.source_platform.trim().toLowerCase()
            : detectSourcePlatform(sourceUrl);

        const result = await run(
            `INSERT INTO wallet_items (title, source_url, source_platform, note) VALUES (?, ?, ?, ?)`,
            [title, sourceUrl, sourcePlatform, note]
        );
...
app.patch("/wallet/:id", async (req, res) => {
    try {
        ...
        const title = typeof req.body.title === "string" ? req.body.title.trim() : current.title;
        const sourceUrl = typeof req.body.source_url === "string" ? req.body.source_url.trim() : current.source_url;
        const note = typeof req.body.note === "string" ? req.body.note.trim() : current.note;
        const status = typeof req.body.status === "string" ? req.body.status.trim() : current.status;
        const sourcePlatform = typeof req.body.source_platform === "string" && req.body.source_platform.trim()
            ? req.body.source_platform.trim().toLowerCase()
            : detectSourcePlatform(sourceUrl);
        await run(`UPDATE wallet_items SET title = ?, source_url = ?, source_platform = ?, note = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [title || current.title, sourceUrl, sourcePlatform, note, status || "new", req.params.id]);
