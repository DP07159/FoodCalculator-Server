...
async function ensureSchema() {
    ...
    await run(`CREATE INDEX IF NOT EXISTS idx_wallet_items_workspace ON wallet_items(workspace_id, created_at DESC)`);
}
...
function validateWalletPayload(payload) {
    ...
    if (!sourceUrl) return { error: "Ein Link ist erforderlich." };
    try {
        const parsed = new URL(sourceUrl);
        if (!["http:", "https:"].includes(parsed.protocol)) throw new Error();
    } catch {
        return { error: "Bitte einen gültigen http(s)-Link angeben." };
    }
    if (!title) return { error: "Ein Titel ist erforderlich." };

    return { value: { sourceUrl, title, note, workspaceId, sourcePlatform: detectSourcePlatform(sourceUrl) } };
}
...
app.get("/wallet", async (req, res) => {
    try {
        ...
        const rows = await all(
            `SELECT * FROM wallet_items WHERE workspace_id = ? AND status != 'deleted' ORDER BY datetime(created_at) DESC, id DESC`,
            [workspaceId]
        );
        res.json(rows);
    } catch (error) {
        console.error("Fehler bei GET /wallet:", error.message);
        res.status(500).json({ error: "Wallet konnte nicht geladen werden" });
    }
});

app.get("/wallet/:id", async (req, res) => {
    try {
        const row = await get(`SELECT * FROM wallet_items WHERE id = ? AND status != 'deleted'`, [req.params.id]);
        if (!row) return res.status(404).json({ error: "Wallet-Eintrag nicht gefunden" });
        res.json(row);
    } catch (error) {
        res.status(500).json({ error: "Wallet-Eintrag konnte nicht geladen werden" });
    }
});
...
app.post("/wallet", async (req, res) => {
    try {
        ...
        const result = await run(
            `INSERT INTO wallet_items (workspace_id, source_url, source_platform, title, note) VALUES (?, ?, ?, ?, ?)`,
            [item.workspaceId, item.sourceUrl, item.sourcePlatform, item.title, item.note]
        );
        ...
        res.status(500).json({ error: "Wallet-Eintrag konnte nicht gespeichert werden" });
    }
});

app.patch("/wallet/:id", async (req, res) => {
    try {
        const current = await get(`SELECT * FROM wallet_items WHERE id = ? AND status != 'deleted'`, [req.params.id]);
        ...
        const validation = validateWalletPayload({
            ...
            workspace_id: req.body.workspace_id ?? current.workspace_id
        });
        if (validation.error) return res.status(400).json({ error: validation.error });
        const item = validation.value;
        await run(
            `UPDATE wallet_items SET workspace_id = ?, source_url = ?, source_platform = ?, title = ?, note = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
            [item.workspaceId, item.sourceUrl, item.sourcePlatform, item.title, item.note, req.params.id]
        );
        res.json(await get(`SELECT * FROM wallet_items WHERE id = ?`, [req.params.id]));
    } catch (error) {
        res.status(500).json({ error: "Wallet-Eintrag konnte nicht aktualisiert werden" });
    }
});

app.delete("/wallet/:id", async (req, res) => {
    try {
        const result = await run(`UPDATE wallet_items SET status = 'deleted', updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [req.params.id]);
        if (result.changes === 0) return res.status(404).json({ error: "Wallet-Eintrag nicht gefunden" });
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: "Wallet-Eintrag konnte nicht gelöscht werden" });
    }
});
