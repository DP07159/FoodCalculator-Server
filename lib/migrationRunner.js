...
const SUPERSEDES_CHECKSUM_PATTERN = /^\s*--\s*@supersedes-checksum\s+(\d{4})\s*$/i;
...
function parseSupersededChecksumVersions(sql) {
    return String(sql || "")
        .split(/\r?\n/)
        .map(line => line.match(SUPERSEDES_CHECKSUM_PATTERN))
        .filter(Boolean)
        .map(match => match[1]);
}
...
async function runMigrations(connection, options = {}) {
    ...
    const appliedNow = [];
    const supersededChecksumVersions = new Map();

    for (const file of files) {
        for (const supersededVersion of parseSupersededChecksumVersions(file.sql)) {
            if (file.version <= supersededVersion) {
                throw new Error(
                    `Migration ${file.filename} darf nur eine ältere Migration per @supersedes-checksum ablösen.`
                );
            }
            supersededChecksumVersions.set(supersededVersion, file.filename);
        }
    }
    ...
    for (const migration of files) {
        ...
        if (existing) {
            if (existing.filename !== migration.filename) {
                throw new Error(`Migration ${migration.version} wurde nachträglich umbenannt (${migration.filename}).`);
            }

            if (existing.checksum !== migration.checksum) {
                const supersedingMigration = supersededChecksumVersions.get(migration.version);
                if (!supersedingMigration) {
                    throw new Error(`Migration ${migration.version} wurde nachträglich verändert (${migration.filename}).`);
                }

                console.warn(
                    `Migration ${migration.version} hat eine abweichende Prüfsumme; ` +
                    `${supersedingMigration} übernimmt die explizite Reparatur. Historische Prüfsumme bleibt unverändert.`
                );
            }
            continue;
        }
